...
def run(algo_id, base_config, base_dir, dataset, spec, data_params={}):
    ...
    algo.fit()
    # create summary
    create_summary(dataset, base_config.device, algo.G, base_config.p, base_config.q, x_real_test, experiment_directory = experiment_directory, num = 1)
    savefig('summary.png', experiment_directory)
    x_fake = create_summary(dataset, base_config.device, algo.G, base_config.p, 8000, x_real_test, one=True, experiment_directory = experiment_directory, num = 2)
...
def get_dataset_configuration(dataset):
    if dataset == 'ECG':
        ...
        # руками менять на другие акции
        #generator = list(((asset, dict(assets = asset)) for asset in ['AAPL', 'NVDA']))
        generator = list(((asset, dict(assets = asset)) for asset in ['AAPL']))
    else:
...
if __name__ == '__main__':
    ...
    # Meta parameters
    parser.add_argument('-base_dir', default='.\exp_5_res', type=str)
    parser.add_argument('-use_cuda', action='store_true')
    parser.add_argument('-device', default=0, type=int)
    parser.add_argument('-num_seeds', default=1, type=int)
    parser.add_argument('-initial_seed', default=0, type=int)
    parser.add_argument('-datasets', default=['HIGH_FREQ'], nargs="+")
