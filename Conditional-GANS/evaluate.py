...
def compute_predictive_score(x_past, x_future, x_fake):
    ...
    model = LinearRegression()
    model.fit(X, Y)  # TSTR
    r2_tstr_linreg = model.score(X_test, Y_test)
    model = LinearRegression()
    model.fit(X_test, Y_test)  # TRTR
    r2_trtr_linreg = model.score(X_test, Y_test)
    # print('Linreg done')

    model = RandomForestRegressor()
    model.fit(X, Y)  # TSTR
    r2_tstr_forrest = model.score(X_test, Y_test)
    model = RandomForestRegressor()
    model.fit(X_test, Y_test)  # TRTR
    r2_trtr_forrest = model.score(X_test, Y_test)
    # print('Forrest done')

    model = MultiOutputRegressor(GradientBoostingRegressor())
    model.fit(X, Y)  # TSTR
    r2_tstr_boosting = model.score(X_test, Y_test)
    model = MultiOutputRegressor(GradientBoostingRegressor())
    model.fit(X_test, Y_test)  # TRTR
    r2_trtr_boosting = model.score(X_test, Y_test)
    # print('Boosting done')

    return dict(r2_tstr_linreg=r2_tstr_linreg, r2_trtr_linreg=r2_trtr_linreg, predictive_score_linreg=np.abs(r2_trtr_linreg - r2_tstr_linreg), \
                r2_tstr_forrest=r2_tstr_forrest, r2_trtr_forrest=r2_trtr_forrest, predictive_score_forrest=np.abs(r2_trtr_forrest - r2_tstr_forrest),
                r2_tstr_boosting=r2_tstr_boosting, r2_trtr_boosting=r2_trtr_boosting, predictive_score_boosting=np.abs(r2_trtr_boosting - r2_tstr_boosting))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-datasets', default=['HIGH_FREQ', 'STOCKS', ], nargs="+")
