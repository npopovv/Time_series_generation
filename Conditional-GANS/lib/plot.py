...
def create_summary(dataset, device, G, lags_past, steps, x_real, one=False, experiment_directory = None, num = None):
    #print(dataset)
    #print('x_real.shape', x_real.shape)
    with torch.no_grad():
        x_past = x_real[:, :lags_past]
        if dataset in ['STOCKS', 'ECG', 'HOGH_FREQ']:
            #x_real  имеет повторяющиеся значения, вроде как 6 здесь нужно
        ...
        pickle_it(x_fake_future, pt.join(experiment_directory,f'x_fake_future_plot_{str(num)}.torch'))
        pickle_it(x_real, pt.join(experiment_directory, f'x_real_plot_{str(num)}.torch'))
        pickle_it(x_p, pt.join(experiment_directory, f'x_p_plot_{str(num)}.torch'))
